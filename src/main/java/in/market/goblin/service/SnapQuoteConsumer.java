/*package in.market.goblin.service;
 ...
 / import in.angelbroking.smartapi.smartstream.models.SmartApiBBSInfo;
 ...
 import java.util.concurrent.Executors;
  ...
  // Saves liveData, bids, and asks to database concurrently using multithreading.

 @Service
 public class SnapQuoteConsumer {
	 private LiveDataRepository liveDataRepository=null;
	 private MarketDepthDataRepository marketDepthDataRepository=null;
	 private final ExecutorService executorService = Executors.newFixedThreadPool(4);
	 // Thread pool for parallel tasks

 // Kafka listener to consume SnapQuote messages.
// @param snapQuote SnapQuote object
@KafkaListener(topics = "snapquote", groupId = "snapquote-group")
public void consumeSnapQuote(MarketUpdateV3 marketUpdate) {
	long startTime = System.nanoTime();
 	//ObjectMapper mapper = new ObjectMapper();
	//MarketUpdateV3 snapQuote = mapper.readValue(snapQuoteJson, MarketUpdateV3.class);

 // Parallel saving with CompletableFuture
	CompletableFuture<Void> liveDataFuture = CompletableFuture.runAsync( () -> saveLiveData(marketUpdate), executorService);
	CompletableFuture<Void> bidsFuture = CompletableFuture.runAsync( () -> saveBids(marketUpdate), executorService);
	CompletableFuture<Void> asksFuture = CompletableFuture.runAsync( () -> saveAsks(marketUpdate), executorService);

 // Ensure all tasks complete or log partial failure
	CompletableFuture.allOf(liveDataFuture, bidsFuture, asksFuture).whenComplete((result, throwable) -> {
		if (throwable != null) {
 			System.err.printf("Error saving SnapQuote data for sequenceNumber: {}: {}", marketUpdate.getCurrentTs(), throwable.getMessage());
		} else {
 		 System.err.printf("Completed saving SnapQuote for sequenceNumber: {}, time: {}ms",
				 marketUpdate.getCurrentTs(), (System.nanoTime() - startTime) / 1_000_000.0);
		} }).exceptionally(throwable -> {
		System.err.printf("Unexpected error for sequenceNumber: {}: {}", marketUpdate.getCurrentTs(), throwable.getMessage());
		return null;
		});
}
...
// @return Saved liveData entity
 @Transactional
 private void saveLiveData(MarketUpdateV3 snapQuote) {
	// Save to parent table (live_data)
	 try { LiveData liveData = new LiveData();
  ...
  /*
	  ...
	  */
		/*   @Transactional
		   private void saveAsks(SnapQuote snapQuote) {
			   try {
